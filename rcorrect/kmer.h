...
static INLINE_FUNCTION void bfc_kmer_append(int k, uint64_t x[4], int c)
{ // IMPORTANT: 0 <= c < 4
...
static INLINE_FUNCTION void bfc_kmer_change(int k, uint64_t x[4], int d, int c) // d-bp from the 3'-end of k-mer; 0<=d<k
{ // IMPORTANT: 0 <= c < 4
	uint64_t t = ~(1ULL<<d);
...
static INLINE_FUNCTION uint64_t bfc_hash_64(uint64_t key, uint64_t mask)
{
...
static INLINE_FUNCTION uint64_t bfc_hash_64_inv(uint64_t key, uint64_t mask)
{
	uint64_t tmp;
...
static INLINE_FUNCTION uint64_t bfc_kmer_hash(int k, const uint64_t x[4], uint64_t h[2])
{
...
static INLINE_FUNCTION void bfc_kmer_hash_inv(int k, const uint64_t h[2], uint64_t y[2])
{
...
static INLINE_FUNCTION char *bfc_kmer_2str(int k, const uint64_t y[2], char *buf)
{
	int l;
	for (l = 0; l < k; ++l)
