...
static INLINE_FUNCTION cnthash_t *get_subhash(const bfc_ch_t *ch, const uint64_t x[2], uint64_t *key)
{
	if (ch->k <= 32) {
...
int bfc_ch_insert(bfc_ch_t *ch, const uint64_t x[2], int is_high, int forced)
{
	...
	utils_atomic_release(&h->lock); // unlock
	return 0;
}
